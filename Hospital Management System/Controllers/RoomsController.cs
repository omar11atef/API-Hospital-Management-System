...
public class RoomsController(IRoomService roomService) : ControllerBase
{
    ...
    [HttpPost("/departments/{departmentId:int}/rooms")]
    public async Task<IActionResult> CreateRoom([FromRoute] int departmentId, [FromBody] RoomRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return result.IsSuccess? CreatedAtAction(nameof(GetRoomById), new { id = result.Value.Id }, result.Value) : result.ToProblem();
    }
    ...
    // Update Room Data:
    [HttpPut("/departments/{departmentId:int}/rooms/{roomId:int}")]
